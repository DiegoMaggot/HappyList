...
export class ShoppingListsPage {
  ...
  getTotal(list: List) {
    return list.products.map(product => product.quantity * product.price)
    .reduce(((totalQuantity, quantity) => totalQuantity += quantity))
    .toFixed(2);
  }
  ...
  goTo(path: string, list: List) {
    ...
    this.router.navigate([`./shopping-lists/${path}/:${list.name}`], navigationExtras);
  }
